---
title: "Akool SDK Best Practice"
description: "Learn how implement Akool SDK step by step"
---

## Overview

When implementing a JavaScript SDK, especially one that interacts with sensitive resources or APIs, it is critical to ensure the security of private keys, tokens, and other sensitive credentials. Exposing such sensitive information in a client-side environment (e.g., browsers) can lead to vulnerabilities, including unauthorized access, token theft, and API abuse. This document outlines best practices for securing private keys and tokens in your Akool SDK implementation while exposing only the necessary session data to the client.
...
1.To get started with the Akool SDK, you just need one html page with lots of elements like below:
